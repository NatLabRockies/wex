...
wxPdfDCImpl::DoDrawEllipticArc(wxCoord x, wxCoord y, wxCoord width, wxCoord height,
    ...
    bool doDraw = GetPen().IsNonTransparent();
    if (doDraw || doFill) {
        m_pdfDocument->SetLineWidth(
                ScaleLogicalToPdfXRel(1)); // pen width != 1 sometimes fools readers when closing paths
        if (doFill) {
...
wxPdfDCImpl::DoDrawRotatedText(const wxString &text, wxCoord x, wxCoord y, double angle) {
    ...
    m_pdfDocument->RotatedText(ScaleLogicalToPdfX(x), ScaleLogicalToPdfY(y), ScaleLogicalToPdfX(x),
                               ScaleLogicalToPdfY(originalY), text, angle);
    SetFont(old);
}
    ...
    if (curPen != wxNullPen) {
        ...
        switch (curPen.GetJoin()) {
            ...
            case wxJOIN_ROUND:
                ...
                break;
            default:
                style.SetLineJoin(wxPDF_LINEJOIN_NONE);
                break;
        }
